...
def downloader(code, key):
    ...
    for each in range(len(code)):
        ...
        try:
            ...
            with open('temp\\{}.temp'.format(filename), 'wb') as f:
                f.write(_get(downloadUrl, verify=False, timeout=10).content)
        except:
            pass
    print('\n下载完毕！正在解压')
    for each in range(len(code)):
    ...
    for each in listdir('PPT'):
        if path.splitext(each)[1] != '.ppt' and path.splitext(each)[1] != '.pptx' and path.splitext(each)[
            1] != '.rar' and not path.isdir('PPT\\' + each):
            remove('PPT\\' + each)
